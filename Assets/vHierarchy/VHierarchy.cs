...
namespace VHierarchy
{
    ...
    {
        /// <summary>
        /// TreeViewController drag selection: Unity 6+ uses a list of <see cref="EntityId"/>;
        /// older versions use <see cref="int"/> instance IDs. Avoid direct casts to <c>List&lt;int&gt;</c>.
        /// </summary>
        static bool DragSelectionListContainsGameObject(IList list, GameObject go)
        {
            if (list == null || !go) return false;

            var iid = go.GetInstanceID();
#if UNITY_6000_0_OR_NEWER
            var goEntityId = go.GetEntityId();
#endif
            foreach (var item in list)
            {
                if (item is int asInt && asInt == iid) return true;
#if UNITY_6000_0_OR_NEWER
                if (item is EntityId eid && eid == goEntityId) return true;
#endif
            }
        ...
        static void GameObjectRowGUI(GameObject go, Rect rowRect)
        {
            ...
            void setState()
            {
                void set_isRowSelected()
                {
                    ...
                    IList dragSelectionList = null;
#if UNITY_2021_1_OR_NEWER
                    var dragSelection = treeViewController?.GetFieldValue("m_DragSelection", false);
                    if (dragSelection != null)
                        dragSelectionList = dragSelection.GetFieldValue("m_List", false) as IList;
#else
                    dragSelectionList = treeViewController?.GetFieldValue<List<int>>("m_DragSelection");
#endif

                    var dragging = dragSelectionList != null && dragSelectionList.Count > 0;

                    isRowSelected = dragging ? DragSelectionListContainsGameObject(dragSelectionList, go) : Selection.Contains(go);
        ...
        {
            var expanded = hierarchyWindow?.GetFieldValue("m_SceneHierarchy", false)
                ?.GetFieldValue("m_TreeViewState", false)
                ?.GetPropertyValue("expandedIDs", false);
            if (expanded is IList rawList)
            {
                var next = new List<int>(rawList.Count);
                foreach (var item in rawList)
                {
                    if (item is int id) next.Add(id);
#if UNITY_6000_0_OR_NEWER
                    else if (item is EntityId eid)
                    {
                        var uo = EditorUtility.EntityIdToObject(eid);
                        if (uo) next.Add(uo.GetInstanceID());
                    }
#endif
                }
                expandedIds = next;
